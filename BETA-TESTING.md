...
Experiment 1 is the separate current full-consensus test network. It enables
all pinned Bitcoin Core transaction/script paths and inherited UTXO spending;
read its [runbook and key-reuse warnings](EXPERIMENT-1.md) before joining. Do
not point an Experiment 0 datadir or activation manifest at Experiment 1.
...
Idena remains live in both experiments. Ownership signatures are public, and
delegation, stake, validation, transactions, and contracts can affect real
IDNA or identity state. In Experiment 1, inherited Bitcoin scripts may also be
controlled by real mainnet keys even though fork balances have no promised
value.
