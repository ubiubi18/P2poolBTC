...
mod tests {
    ...
    struct IdenaDnaSignDoubleHashVector {
        format: String,
        value: String,
    ...
    #[test]
    fn idena_go_double_hash_vector_recovers_expected_address() {
        // Generated by idena-go's dna_sign implementation; the fixture contains no private key.
        let vector: IdenaDnaSignDoubleHashVector = serde_json::from_str(include_str!(
            "../../../testdata/idena_dna_sign_double_hash.json"
        ))
        .unwrap();

        assert_eq!(vector.format, "doubleHash");
        assert_eq!(
            hex::encode(keccak256(vector.value.as_bytes())),
            vector.first_hash_hex
        );
        assert_eq!(
            hex::encode(idena_signin_hash(&vector.value)),
            vector.signing_hash_hex
        );
        assert_eq!(
            recover_idena_signin_address(&vector.value, &vector.signature_hex).unwrap(),
            vector.address
        );
    }
