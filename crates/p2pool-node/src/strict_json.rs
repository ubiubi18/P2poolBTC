...
pub(crate) fn from_slice<T: DeserializeOwned>(payload: &[u8]) -> serde_json::Result<T> {
    let strict: StrictJson = serde_json::from_slice(payload)?;
    serde_json::from_value(strict.0)
}

pub(crate) fn from_str<T: DeserializeOwned>(payload: &str) -> serde_json::Result<T> {
