...
def hash_regular_file(path: Path) -> tuple[str, int]:
    try:
        ...
        raise GateError(f"cannot inspect contract artifact: {path}") from exc
    require(path.is_file() and not path.is_symlink(), "contract artifact must be a regular non-symlink file")
    ...
    with path.open("rb") as stream:
        for chunk in iter(lambda: stream.read(1024 * 1024), b""):
            digest.update(chunk)
            size += len(chunk)
    require(size == metadata.st_size, "contract artifact changed while hashing")
...
def main() -> int:
    ...
    require((idena_go / "vm/wasm/governance_contract_integration_test.go").is_file(), "idena-go runtime integration test is missing")
    ...
    environment["IDENA_GOVERNANCE_WASM_SHA256"] = expected_sha256
    command = [
        "go",
        "test",
        "./vm/wasm",
        "-run",
        "^TestGovernanceContractProductionRuntimeDeterminism$",
        "-count=1",
        "-v",
    ]
    try:
