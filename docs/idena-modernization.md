...
indexer or timer. The health unit is replaced as a complete unit because
`RequiresMountsFor` dependencies generated by the legacy SSD unit cannot be
reliably removed by a later drop-in. The previous unit is retained at
`/var/lib/pohw-p2pool/runtime-backup/pohw-health-status.service`.
