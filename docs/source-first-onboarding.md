...
Idena itself remains live. Signing the ownership challenge publishes a durable
proof of control, while separate delegation, stake, validation, transaction,
or contract actions can affect real IDNA or identity state. The source-first
flow never needs an Idena private key, node key, backup, password, or API key.

This onboarding flow is for frozen Experiment 0. Experiment 1 uses a separate
source-built Bitcoin Core fork and allows inherited UTXO spends. Its source
build does not make Bitcoin key reuse safe; follow the key-risk boundary in
[`EXPERIMENT-1.md`](../EXPERIMENT-1.md).
